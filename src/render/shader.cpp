...
Shader::Shader(const char *vert_data, const char *frag_data) : id(-1), texture_counter(0) {
	// compile shaders
	const auto vert = compileShader(true, vert_data), frag = compileShader(false, frag_data);
...
// bit dodgy
void Shader::setupTexture(const char *texture_name, const char *filename, bool has_transparency) {
	...
	SHADER_CHECK_IMAGE_LOADED(data);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, second_color_value, GL_UNSIGNED_BYTE,
							 data);
	glGenerateMipmap(GL_TEXTURE_2D);

	stbi_image_free(data);

	// bind the texture sampler uniform to correspondent texture number for this program
	// ALWAYS USE PROGRAM BEFORE SETTING UNIFORMS
	use();
